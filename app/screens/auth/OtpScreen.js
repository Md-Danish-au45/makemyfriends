...
const OtpScreen = ({ navigation, route }) => {
  const { confirmation } = route.params; // Get the confirmation object
  const [otp, setOtp] = useState(new Array(6).fill(''));
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Confirm the OTP
      await confirmation.confirm(code); // Use confirmation directly
      navigation.navigate('EmailVerificationScreen'); // Navigate on success
    } catch (error) {
